...
**[MUST] viewport-single-source — one viewport, declared in the PROJECT's `use`, read once by the fixture.** Put `viewport` inside the `projects[]` entry AFTER the `devices[...]` spread — a top-level `use.viewport` is clobbered by the device's own 1280×720. The fixture reads that one value and feeds all three consumers: Stagehand's `localBrowserLaunchOptions.viewport`, `browser.newContext({ viewport })`, and `recordVideo.size`. Never hardcode a second size anywhere (the old template shipped `video.size: 1280×720` against a 1920 page, so every video was a squashed rescale).
...
- **`trace: 'on'` produces NO TRACE, ever.** The runner's artifacts instrumentation has already started tracing on your manually-created context (your own `ctx.tracing.start()` throws *"Tracing has been already started"* on both browser paths), but its teardown only runs for contexts IT created, so the recording is discarded on close. Don't start it — mark the context and **stop it into a file** in teardown (`ctx.tracing.stop({ path })`, then `existsSync` → `testInfo.attach`). Collapse modes to on/off: the `retain-on-*` variants need a failure verdict that isn't known at start.
