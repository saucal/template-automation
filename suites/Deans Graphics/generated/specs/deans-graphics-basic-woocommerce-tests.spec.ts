// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "Deans Graphics - Basic WooCommerce Tests"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { extractUserFromEmail, login, placeOrderElement, register, scrollFullPage, wooCommerceCheckoutTemplate } from '../helpers/common-steps-for-all-projects';
...
test.describe('Deans Graphics - Basic WooCommerce Tests', () => {
  ...
  test('02 - Registration', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
  ...
  test('03 - Admin registration approval', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
  ...
  test('04 - Login, My Account links with Approved user', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
    ...
    await page.locator(`xpath=//a[contains(text(), "Downloads")]`).or(page.locator(`a[href*="/growmark-display-program/my-account/downloads/"]`)).filter({ visible: true }).first().click({ force: true });
    await expect(page.locator(`.wc-block-components-notice-banner__content`).or(page.locator(`.woocommerce-info`)).first()).toContainText(`No downloads available yet.`);
    await page.locator(`xpath=//a[contains(text(), "Addresses")]`).or(page.locator(`a[href*="/growmark-display-program/my-account/edit-address/"]`)).filter({ visible: true }).first().click({ force: true });
    ...
    await page.locator(`a[href*="/growmark-display-program/my-account/edit-account/"]`).filter({ visible: true }).first().click({ force: true });
    await expect(page.locator(`form.woocommerce-EditAccountForm > div.clear:nth-of-type(1)`)).not.toHaveCount(0);
    await page.locator(`a[href*="/growmark-display-program/my-account/customer-logout/?_wpnonce="]`).filter({ visible: true }).first().click({ force: true });
    expect(await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); return window.location.href === 'https://staging-deansgraphics.kinsta.cloud/hello/my-account/?fromsite=5&loop=1'
 }, vars)).toBeTruthy();
  ...
  test('08 - Checkout page', async ({ page }) => {
    ...
    await login(page, vars);
    await page.locator(`a.cart-contents`).first().hover();
    await page.locator(`xpath=//a[contains(text(), "Checkout")]`).or(page.locator(`a[href*="/growmark-display-program/checkout/"]`)).filter({ visible: true }).first().click({ force: true });
  });

  test('09 - Place order', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
    ...
    vars.orderNum = ((await page.locator(`li.woocommerce-order-overview__order.order > strong`).textContent()) ?? '').trim();
...
${vars.phone ?? ''}`);
    ...
    await page.locator(`xpath=//a[contains(text(), "#${vars.orderNum ?? ''}")]`).or(page.locator(`td.woocommerce-orders-table__cell.woocommerce-orders-table__cell-order-number > a[href*="/growmark-display-program/my-account/view-order/${vars.orderNum ?? ''}/"]`)).filter({ visible: true }).first().click({ force: true });
...
${vars.phone ?? ''}`);
  ...
  test('10 - “Forgot password?” flow', async ({ page }) => {
    ...
    await page.locator(`#menu-primary-menu > li.menu-item.menu-item-type-custom.menu-item-object-custom:nth-of-type(6) > a[href*="/hello/"]`).or(page.locator(`#menu-item-9247 > a`)).filter({ visible: true }).first().click({ force: true });
    ...
    await page.locator(`a[href*="https://staging-deansgraphics.kinsta.cloud/hello/my-account/lost-password/?key="]`).or(page.locator(`a[href*="https://staging-deansgraphics.kinsta.cloud/wp-login.php?action=rp&key="]`)).filter({ visible: true }).first().click({ force: true });
    try { await page.locator(`#password_1`).first().fill(`${vars.pass2 ?? ''}`); } catch { await page.locator(`#password_1`).first().selectOption(`${vars.pass2 ?? ''}`); }
    try { await page.locator(`#password_2`).first().fill(`${vars.pass2 ?? ''}`); } catch { await page.locator(`#password_2`).first().selectOption(`${vars.pass2 ?? ''}`); }
    ...
    try {
      await page.goto(`https://staging-deansgraphics.kinsta.cloud/`);
      await page.waitForLoadState('load');
  ...
  test('Mobile Menu', async ({ page }) => {
    ...
    try {
      await page.locator(`#site-navigation .menu-toggle`).filter({ visible: true }).first().click({ force: true });
    } catch { /* optional step: click */ }
    await expect(page.locator(`#site-navigation .handheld-navigation .menu`).first()).toBeVisible();
  });
