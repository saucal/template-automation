...
return prodDesc }, vars);
    ...
    await expect(page.locator(`#shipping_phone`).first()).toHaveText(`${vars.phone ?? ''}`);
...
return price === subtotal 
    ...
    await page.locator(`xpath=//span[contains(text(), "Step 3. Payment (Click Here)")]`).or(page.locator(`a[href="#1711561261565-26c82784-a5ba"] > .vc_tta-title-text`)).filter({ visible: true }).first().click({ force: true });
    {
      const _lbl = page.locator(`label[for="payment_method_ppcp-credit-card-gateway"]`).or(page.locator(`label[for="payment_method_ppcp-card-button-gateway"]`)).filter({ visible: true });
      if (await _lbl.count() > 0) { await _lbl.first().click(); }
      else { await page.locator(`#payment_method_ppcp-credit-card-gateway`).or(page.locator(`#payment_method_ppcp-card-button-gateway`)).filter({ visible: true }).first().click({ force: true }); }
    }
    await blockUI(page, vars);
