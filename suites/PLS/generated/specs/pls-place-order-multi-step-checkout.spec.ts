// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "PLS - Place Order - Multi-Step checkout"
...
test.describe('PLS - Place Order - Multi-Step checkout', () => {
...
const price = firstProductTop.querySelector(
  ".price .amount"
)

const unitPriceComplete = price.textContent
...
return multiSteps !== 'yes' })()) {
    ...
    try { await page.locator(`#billing-pls-core-middle_name`).first().fill(`${vars.midName ?? ''}`); } catch { await page.locator(`#billing-pls-core-middle_name`).first().selectOption(`${vars.midName ?? ''}`); }
...
return element !== null && element !== undefined }, vars)) {
    ...
    await page.locator(`xpath=//button[contains(text(),'Next')]`).or(page.locator(`button.pls-checkout-steps__nav-btn.pls-checkout-steps__nav-btn--next`)).filter({ visible: true }).first().click({ force: true });
    await page.waitForTimeout(3000);
    {
      const _lbl = page.locator(`label[for="checkbox-control-3"]`).filter({ visible: true });
      if (await _lbl.count() > 0) { await _lbl.first().click(); }
      else { await page.locator(`div:nth-child(2) > div.wc-block-components-checkbox input`).or(page.locator(`#checkbox-control-3`)).filter({ visible: true }).first().click({ force: true }); }
    }
    await page.locator(`div:nth-of-type(2) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).filter({ visible: true }).first().click({ force: true });
    try { await page.locator(`div:nth-of-type(2) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).first().fill(`${vars.credentials ?? ''}`); } catch { await page.locator(`div:nth-of-type(2) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).first().selectOption(`${vars.credentials ?? ''}`); }
    try { await page.locator(`div:nth-of-type(4) > div.pls-field-first_name > input`).first().fill(`${vars.firstName2 ?? ''}`); } catch { await page.locator(`div:nth-of-type(4) > div.pls-field-first_name > input`).first().selectOption(`${vars.firstName2 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(4) > div.pls-field-middle_name > input`).first().fill(`${vars.midName2 ?? ''}`); } catch { await page.locator(`div:nth-of-type(4) > div.pls-field-middle_name > input`).first().selectOption(`${vars.midName2 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(4) > div.pls-field-last_name > input`).first().fill(`${vars.lastName2 ?? ''}`); } catch { await page.locator(`div:nth-of-type(4) > div.pls-field-last_name > input`).first().selectOption(`${vars.lastName2 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(4) > div.pls-field-email > input`).first().fill(`${vars.email2 ?? ''}`); } catch { await page.locator(`div:nth-of-type(4) > div.pls-field-email > input`).first().selectOption(`${vars.email2 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(4) > div.pls-field-phone > input`).first().fill(`${vars.phone2 ?? ''}`); } catch { await page.locator(`div:nth-of-type(4) > div.pls-field-phone > input`).first().selectOption(`${vars.phone2 ?? ''}`); }
    await page.locator(`div:nth-of-type(4) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).filter({ visible: true }).first().click({ force: true });
    try { await page.locator(`div:nth-of-type(4) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).first().fill(`${vars.credentials2 ?? ''}`); } catch { await page.locator(`div:nth-of-type(4) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).first().selectOption(`${vars.credentials2 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(6) > div.pls-field-first_name > input`).first().fill(`${vars.firstName3 ?? ''}`); } catch { await page.locator(`div:nth-of-type(6) > div.pls-field-first_name > input`).first().selectOption(`${vars.firstName3 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(6) > div.pls-field-middle_name > input`).first().fill(`${vars.midName3 ?? ''}`); } catch { await page.locator(`div:nth-of-type(6) > div.pls-field-middle_name > input`).first().selectOption(`${vars.midName3 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(6) > div.pls-field-last_name > input`).first().fill(`${vars.lastName3 ?? ''}`); } catch { await page.locator(`div:nth-of-type(6) > div.pls-field-last_name > input`).first().selectOption(`${vars.lastName3 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(6) > div.pls-field-email > input`).first().fill(`${vars.email3 ?? ''}`); } catch { await page.locator(`div:nth-of-type(6) > div.pls-field-email > input`).first().selectOption(`${vars.email3 ?? ''}`); }
    try { await page.locator(`div:nth-of-type(6) > div.pls-field-phone > input`).first().fill(`${vars.phone3 ?? ''}`); } catch { await page.locator(`div:nth-of-type(6) > div.pls-field-phone > input`).first().selectOption(`${vars.phone3 ?? ''}`); }
    await page.locator(`div:nth-of-type(6) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).filter({ visible: true }).first().click({ force: true });
    try { await page.locator(`div:nth-of-type(6) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).first().fill(`${vars.credentials3 ?? ''}`); } catch { await page.locator(`div:nth-of-type(6) > .pls-field-credentials > div > div > select[aria-label="Credentials"]`).first().selectOption(`${vars.credentials3 ?? ''}`); }
    expect(await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); function checkCredentialsPresence() {
  const expected = [
    'Admin', 'CDA', 'CRDH', 'DA', 'DDS', 'DH', 'DMD', 
    'FTD', 'O.Mgr', 'RDA', 'RDH', 'Recptn', 'RN', 'Other'
  ];

  const selects = Array.from<any>(document.querySelectorAll('.pls-field-credentials'));
...
// Run it
return checkCredentialsPresence(); }, vars)).toBeTruthy();
    await page.locator(`xpath=//button[contains(text(), "Next")]`).or(page.locator(`button[type="button"].pls-checkout-steps__nav-btn.pls-checkout-steps__nav-btn--next`)).filter({ visible: true }).first().click({ force: true });
...
return element !== null && element !== undefined }, vars)) {
    ...
    try { await page.locator(`#order_comments`).or(page.locator(`.wc-block-checkout__order-notes .wc-block-components-textarea`)).first().fill(`Testing Notes`); } catch { await page.locator(`#order_comments`).or(page.locator(`.wc-block-checkout__order-notes .wc-block-components-textarea`)).first().selectOption(`Testing Notes`); }
    {
      ...
      if (await _lbl.count() > 0) { await _lbl.first().click(); }
      else { await page.locator(`xpath=//*[contains(text(), "Place order")]`).or(page.locator(`#place_order`)).or(page.locator(`.wc-block-components-checkout-place-order-button__text`)).filter({ visible: true }).first().click({ force: true }); }
    }
    ...
    await expect(page.locator(`.woocommerce-customer-details > address`).first()).toHaveText(`${vars.firstName ?? ''} ${vars.midName ?? ''} ${vars.lastName ?? ''}
${vars.company ?? ''}
${vars.street ?? ''}
${vars.street2 ?? ''}
...
${vars.email ?? ''}`);
  ...
  test('02 - Place order - Email', async ({ page }) => {
    ...
    if (vars.discount !== '') {
      await expect(page.locator(`tfoot > tr:nth-of-type(2) > td.td > .woocommerce-Price-amount.amount`).or(page.locator(`tbody > tr.order-totals.order-totals-discount > td`)).first()).toContainText(`${vars.discount ?? ''}`);
    }
    if (vars.discount == '') {
    ...
    await expect(page.locator(`td:nth-of-type(1) > address.address`).first()).toHaveText(`${vars.firstName ?? ''} ${vars.midName ?? ''} ${vars.lastName ?? ''}
${vars.company ?? ''}
${vars.street ?? ''}
${vars.street2 ?? ''}
${vars.city ?? ''}, ${vars.state ?? ''} ${vars.zipCode ?? ''}
...
${vars.email ?? ''}`);
    ...
    await page.locator(`xpath=//a[contains(text(), "Your new course at Professional Learning Services")]`).or(page.locator(`xpath=//a[contains(text(), "Your account on Professional Learning Services")]`)).or(page.locator(`xpath=//a[contains(text(), "Your Professional Learning Services account has been created")]`)).filter({ visible: true }).first().click({ force: true });
    try {
    ...
    await expect(page.locator(`#body_content_inner > p:nth-of-type(2)`).first()).toContainText(`${vars.firstName ?? ''} ${vars.lastName ?? ''} (${vars.email ?? ''}) just purchased a course for you at Professional Learning Services`);
    try {
    ...
    await page.locator(`xpath=//a[contains(text(), "Your account on Professional Learning Services")]`).or(page.locator(`xpath=//a[contains(text(), "Your Professional Learning Services account has been created")]`)).filter({ visible: true }).first().click({ force: true });
    try {
      await expect(page.locator(`#body_content_inner > p:nth-of-type(2)`).first()).toContainText(`${vars.firstName ?? ''} ${vars.lastName ?? ''} (${vars.email ?? ''}) just purchased a course for you at Professional Learning Services`);
    } catch { /* optional step: assertTextPresent */ }
    try {
  ...
  test('03 - Place order - Backend', async ({ page }) => {
    ...
    await expect(page.locator(`div.order_data_column:nth-of-type(2) > .address > p:nth-of-type(1)`).first()).toHaveText(`${vars.firstName ?? ''} ${vars.midName ?? ''} ${vars.lastName ?? ''}
${vars.company ?? ''}
${vars.street ?? ''}
${vars.street2 ?? ''}
${vars.city ?? ''}, ${vars.state ?? ''} ${vars.zipCode ?? ''}`);
...
Testing Notes`);
    ...
    if (vars.days !== '') {
      await expect(page.locator(`tbody#order_line_items > tr:nth-of-type(1) a[href*="/wp-admin/post.php?post="]`).first()).toContainText(`${vars.days ?? ''}`);
    }
    ...
    await expect(page.locator(`#order_line_items > tr > td.line_cost > div.view > span > bdi`).first()).toHaveText(`${vars.subtotal ?? ''}`);
    ...
    await expect(page.locator(`#order_line_items > tr > td.name > div.pls-confirmation-summary__participants > div:nth-child(1)`).first()).toHaveText(`Participant #1
Full Name: ${vars.firstName ?? ''} ${vars.midName ?? ''} ${vars.lastName ?? ''}
Email: ${vars.email ?? ''}
Credentials: ${vars.credentials ?? ''}
Phone: ${vars.phone ?? ''}`);
    await expect(page.locator(`#order_line_items > tr > td.name > div.pls-confirmation-summary__participants > div:nth-child(2)`).first()).toContainText(`Participant #2
Full Name: ${vars.firstName2 ?? ''} ${vars.midName2 ?? ''} ${vars.lastName2 ?? ''}
Email: ${vars.email2 ?? ''}
Credentials: ${vars.credentials2 ?? ''}
Phone: ${vars.phone2 ?? ''}`);
    await expect(page.locator(`#order_line_items > tr > td.name > div.pls-confirmation-summary__participants > div:nth-child(3)`).first()).toContainText(`Participant #3
