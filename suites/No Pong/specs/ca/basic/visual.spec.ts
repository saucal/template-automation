...
//
// Refresh baselines:  npx playwright test specs/ca/basic/visual.spec.ts --project=ca-develop --update-snapshots
...
// Page slugs copied from the AU suite — NOT yet confirmed against the ca subsite; verify on first run.
...
test.describe('CA Visual — content pages', { tag: ['@plugin:woocommerce'] }, () => {
...
test.describe('CA Visual — cart & checkout', { tag: ['@plugin:woocommerce'] }, () => {
  test('visual: cart', async ({ shopperPage: page }) => {
    await page.goto('./');
    await dismissPopups(page);
    // Empty slug → first non-bundle product on the shop page (intended).
    await addToCart(page, 'ca', { kind: 'simple', slug: '', qty: 1 });
...
test.describe('CA Visual — slider autoplay', { tag: ['@plugin:woocommerce'] }, () => {
...
test.describe('CA Visual — mobile add-to-cart popup', { tag: ['@plugin:woocommerce'] }, () => {
...
test.describe('CA Basic — FAQ accordion', { tag: ['@plugin:woocommerce'] }, () => {
