// NP-US-PO — Place-order serial chain. Replaces GI New-User 01-05 (place /
...
test.describe.serial('NP-US-PO — Place order', { tag: TAGS }, () => {
  test('NP-US-PO-01 — new-user order (Stripe) + full parity', async ({ shopperPage, adminPage, emailPage }) => {
  ...
  test('NP-US-PO-02 — full refund (Stripe) + refund email', async ({ shopperPage: _keepalive, adminPage, emailPage }) => {
    test.skip(process.env.REFUND_PROJECT !== 'us', 'refund is gated to a single region (REFUND_PROJECT)');
    if (!chain.capture) throw new Error('NP-US-PO-01 must have produced an order before the refund runs');

    await performAndAssertRefund(adminPage, chain.capture!, newUser);
    await assertRefundEmail(emailPage, chain.capture!);
  });

  test('NP-US-PO-03 — logged-in user reorders (Stripe)', async ({ shopperPage, adminPage, emailPage }) => {
    if (!chain.email) throw new Error('NP-US-PO-01 must have created an account to reuse');
  ...
  test('NP-US-PO-04 — non-wholesale user is blocked from wholesale pricing', async ({ shopperPage }) => {
    if (!chain.email) throw new Error('NP-US-PO-01 must have created an account to log in with');
    await loginAccount(shopperPage, chain.email!, PASSWORD);
