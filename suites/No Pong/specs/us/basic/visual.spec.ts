...
//
// Refresh baselines:  npx playwright test specs/us/basic/visual.spec.ts --project=us-develop --update-snapshots
...
// Page slugs copied from the AU suite — NOT yet confirmed against the us subsite; verify on first run.
...
test.describe('US Visual — content pages', { tag: ['@plugin:woocommerce'] }, () => {
...
test.describe('US Visual — cart & checkout', { tag: ['@plugin:woocommerce'] }, () => {
  test('visual: cart', async ({ shopperPage: page }) => {
    await page.goto('./');
    await dismissPopups(page);
    // Empty slug → first non-bundle product on the shop page (intended).
    await addToCart(page, 'us', { kind: 'simple', slug: '', qty: 1 });
...
test.describe('US Visual — slider autoplay', { tag: ['@plugin:woocommerce'] }, () => {
...
test.describe('US Visual — mobile add-to-cart popup', { tag: ['@plugin:woocommerce'] }, () => {
...
test.describe('US Basic — FAQ accordion', { tag: ['@plugin:woocommerce'] }, () => {
