...
test.describe('US Quantity limits', { tag: ['@plugin:woocommerce'] }, () => {
  test('NP-US-LIM-01 — tin per-item cap clamps an over-limit quantity', async ({ shopperPage: page }) => {
    await addToCartById(page, TIN_PRODUCT_ID);
    await setCartQtyAndUpdate(page, 50);
    await assertQuantityLimit(page, { clampedQty: '49' });
  });

  test('NP-US-LIM-02 — 85g combined-weight cap clamps the cart', async ({ shopperPage: page }) => {
    await addToCartById(page, TIN_PRODUCT_ID);
    await setCartQtyAndUpdate(page, 50);
