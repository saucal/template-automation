// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "MC - CC - Hosted Session - Capture - Classic"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { addDownloadProductToCart, addPhysicalProductToCart, addVirtualProductToCart, checkTransaction, checkTranscationIsPresentOnOrderBackend, fillCheckout, getWooOrderDetails, login, placeOrder, useMASTERChallenge, useMASTERFrictionlessACSNo, useMASTERFrictionlessACSYes, verifyTransactionOnLogs } from '../helpers/mc-common-steps-for-tests';
...
test.describe('MC - CC - Hosted Session - Capture - Classic', () => {
  ...
  test('MC-009 - Logged user Pay with saving new CC ', async ({ page }) => {
    ...
    vars.savedCC = `no`;
    vars.user = `old`;
    await useMASTERChallenge(page, vars);
    await login(page, vars);
    await addDownloadProductToCart(page, vars);
    await placeOrder(page, vars);
  });
