// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "MC - CC - Hosted Session - Session blocks"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { blockUI } from '../helpers/common-steps-for-all-projects';
import { addPhysicalProductToCart, fillCheckout } from '../helpers/mc-common-steps-for-tests';
...
test.describe('MC - CC - Hosted Session - Session blocks', () => {
  ...
  test('MC-002 - Not filling CC info (CC - Exp - CVC)', async ({ page }) => {
    ...
    await blockUI(page, vars);
    await expect(page.locator(`.woocommerce-error`).or(page.locator(`div.wc-block-components-notices > div > div > div`)).first()).toHaveText(`Card number invalid or missing. Expiry month invalid or missing. Expiry year invalid or missing`);
  });
  ...
  test('MC-003 - Invalid missing CC', async ({ page }) => {
    ...
    try {
      {
        const _lbl = page.locator(`label[for="payment_method_acme"]`).or(page.locator(`label[for="radio-control-wc-payment-method-options-acme"]`)).or(page.locator(`label[for="radio-control-wc-payment-method-options-mastercard_merchant_cloud"]`)).filter({ visible: true });
        if (await _lbl.count() > 0) { await _lbl.first().click(); }
        else { await page.locator(`#payment_method_acme`).or(page.locator(`#radio-control-wc-payment-method-options-acme`)).or(page.locator(`#radio-control-wc-payment-method-options-mastercard_merchant_cloud`)).filter({ visible: true }).first().click({ force: true }); }
      }
    } catch { /* optional step: click */ }
    await blockUI(page, vars);
    ...
    await blockUI(page, vars);
    await expect(page.locator(`.woocommerce-error`).or(page.locator(`div.wc-block-components-notices > div > div > div`)).first()).toContainText(`Card number invalid or missing`);
  ...
  test('MC-003 - Invalid missing CVC', async ({ page }) => {
    ...
    await page.locator(`xpath=//div[contains(text(), "Place Order")]`).or(page.locator(`.wc-block-components-checkout-place-order-button`)).filter({ visible: true }).first().click({ force: true });
    await blockUI(page, vars);
    await expect(page.locator(`.woocommerce-error`).or(page.locator(`div.wc-block-components-notices > div > div > div`)).first()).toContainText(`Security code is invalid or missing`);
    try { await page.locator(`iframe[src*="test-gateway.mastercard.com"]`).first().contentFrame().locator(`#securityCode`).first().fill(``); } catch { await page.locator(`iframe[src*="test-gateway.mastercard.com"]`).first().contentFrame().locator(`#securityCode`).first().selectOption(``); }
    {
      const _lbl = page.locator(`label[for="place_order"]`).filter({ visible: true });
      if (await _lbl.count() > 0) { await _lbl.first().click(); }
      else { await page.locator(`xpath=//button[contains(text(), "Place order")]`).or(page.locator(`#place_order`)).or(page.locator(`xpath=//div[contains(text(), "Place Order")]`)).or(page.locator(`.wc-block-components-checkout-place-order-button`)).filter({ visible: true }).first().click({ force: true }); }
  ...
  test('MC-003 - Invalid missing Exp Month', async ({ page }) => {
    ...
    await blockUI(page, vars);
    await expect(page.locator(`.woocommerce-error`).or(page.locator(`div.wc-block-components-notices > div > div > div`)).first()).toContainText(`Expiry month invalid or missing`);
    {
      let _ok = false;
  ...
  test('MC-003 - Invalid missing Exp Year', async ({ page }) => {
    ...
    await blockUI(page, vars);
    await expect(page.locator(`.woocommerce-error`).or(page.locator(`div.wc-block-components-notices > div > div > div`)).first()).toContainText(`Expiry year invalid or missing`);
    {
      let _ok = false;
