// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "McKeen - Order - Approved transaction"
...
test.describe('McKeen - Order - Approved transaction', () => {
...
return qty
    ...
    try { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="exp-date"]`).first().fill(`04 / 25`); } catch { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="exp-date"]`).first().selectOption(`04 / 25`); }
...
${vars.city ?? ''} ${vars.state ?? ''} ${vars.zipCode ?? ''}`);
  ...
  test('Place order - 02 - Backend Admin', async ({ page }) => {
    ...
    await expect(page.locator(`li.note.system-note:nth-of-type(2) > .note_content > p`).first()).toContainText(`Order status changed from Charge to Processing.`);
    await expect(page.locator(`li.note.system-note:nth-of-type(1) > .note_content > p`).first()).toContainText(`Stripe charge complete (Charge ID: `);
