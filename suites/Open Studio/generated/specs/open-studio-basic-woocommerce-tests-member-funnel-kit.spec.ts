...
return price[0] }, vars);
    await page.locator(`a[href*="/checkout/?add-to-cart=17&quantity=1"]`).or(page.locator(`a[href*="/checkout/?add-to-cart=1607914&quantity=1"]`)).filter({ visible: true }).first().click({ force: true });
...
return '$'+totalPrice   === `${vars.unitPrice}` }, vars)).toBeTruthy();
    ...
    await expect(page.locator(`input[checked='checked']#os-sub-variant-17`).or(page.locator(`input[checked='checked']#os-sub-variant-1607914`))).not.toHaveCount(0);
    await expect(page.locator(`input#os-sub-variant-16[checked="checked"]`).or(page.locator(`input#os-sub-variant-1607913[checked="checked"]`))).toHaveCount(0);
...
return price[0] }, vars);
    await page.locator(`a[href*="/checkout/?add-to-cart=16&quantity=1"]`).or(page.locator(`a[href*="/checkout/?add-to-cart=1607913&quantity=1"]`)).filter({ visible: true }).first().click({ force: true });
    await nextPaymentDate(page, vars);
    ...
    await expect(page.locator(`input[checked='checked']#os-sub-variant-16`).or(page.locator(`input[checked='checked']#os-sub-variant-1607913`))).not.toHaveCount(0);
    await expect(page.locator(`input#os-sub-variant-17[checked="checked"]`).or(page.locator(`input#os-sub-variant-1607914[checked="checked"]`))).toHaveCount(0);
...
OPEN STUDIO`);
    ...
    await expect(page.locator(`mark.order-number`).first()).toHaveText(`${vars.orderNumber ?? ''}`);
    await expect(page.locator(`mark.order-status`).first()).toContainText(`Completed`);
    await expect(page.locator(`a[href*="?attribute_frequency=${vars.freq ?? ''}"]`).first()).toContainText(`Open Studio - ${vars.freq ?? ''}`);
    try {
...
${vars.email1 ?? ''}`);
    ...
    await expect(page.locator(`a[href*="/product/"]`).first()).toContainText(`Open Studio - ${vars.freq ?? ''}`);
    try {
      await expect(page.locator(`li > p`).first()).toContainText(`${vars.freq ?? ''}`);
    } catch { /* optional step: assertTextPresent */ }
