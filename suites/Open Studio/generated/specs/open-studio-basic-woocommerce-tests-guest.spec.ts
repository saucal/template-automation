...
Join now`);
  ...
  test('18 - Join OS Membership - Checkout page', async ({ page }) => {
    ...
    await expect(page.locator(`a[href*="/checkout/?add-to-cart=17&quantity=1"]`).or(page.locator(`a[href*="/checkout/?add-to-cart=1607914&quantity=1"]`)).first()).toContainText(`Start Free Trial`);
    await page.locator(`a[href*="/checkout/?add-to-cart=1607914&quantity=1"]`).filter({ visible: true }).first().click({ force: true });
  });
