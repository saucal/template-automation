// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "Open Studio - Basic WooCommerce Tests - Member"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { blockUI, placeOrderElement } from '../helpers/common-steps-for-all-projects';
import { addSingleCourse, adminLogin, checkOrderOnBackend, extractUserFromEmail, fillCC, fillingSurveyInWelcomePage, login, nextPaymentDate } from '../helpers/open-studio-common-steps';
...
test.describe('Open Studio - Basic WooCommerce Tests - Member', () => {
  ...
  test('01 - Join OS Membership - 01 Submiting Survey', async ({ page }) => {
    await page.goto(`/`);
...
return '$'+totalPrice   === `${vars.unitPrice}` }, vars)).toBeTruthy();
    ...
    await expect(page.locator(`#order_summary_field > div > table > tfoot > tr:nth-child(2) > td > strong > span > bdi`).or(page.locator(`#order_review > div.os-checkout-order-review > table > tfoot > tr:nth-child(2) > td > strong > span > bdi`)).first()).toContainText(`$0.00`);
...
Next renewal:${vars.nextPayment ?? ''}`);
    ...
    await expect(page.locator(`strong > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`$0.00`);
    await fillingSurveyInWelcomePage(page, vars);
    await expect(page.locator(`a[href*="/product/open-studio/?attribute_frequency=Annually"]`).first()).toContainText(`Open Studio - Annually`);
    ...
    await expect(page.locator(`td.woocommerce-table__product-total > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`$0.00`);
...
${vars.email ?? ''}`);
  ...
  test('02 - Join OS Membership - Ignoring Survey', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
...
return formattedDate }, vars);
    ...
    vars.totalPrice = ((await page.locator(`#order_review > div.os-checkout-order-review > table > tbody > tr > td.product-total > span > span.woocommerce-Price-amount.amount > bdi`).textContent()) ?? '').trim();
    ...
    await expect(page.locator(`td.product-name`).first()).toContainText(`Open Studio - ${vars.freq ?? ''}`);
    await expect(page.locator(`#order_review > div.os-checkout-order-review > table > tfoot > tr:nth-child(2) > td > strong > span > bdi`).first()).toHaveText(`$0.00`);
    await expect(page.locator(`#order_review > div.os-checkout-order-review > table > tfoot > tr.order-total.recurring-total > td`).first()).toContainText(`${vars.totalPrice ?? ''}
/ month
Next renewal:${vars.nextPayment ?? ''}`);
    ...
    await page.waitForLoadState('load');
    vars.orderNumber = ((await page.locator(`.order > strong`).textContent()) ?? '').trim();
    await expect(page.locator(`.email > strong`).first()).toHaveText(`${vars.email1 ?? ''}`);
    await expect(page.locator(`strong > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`$0.00`);
...
Join now`);
  ...
  test('22 - Purchase Single Course', async ({ page }) => {
    ...
    await expect(page.locator(`td.product-name`).first()).toHaveText(`${vars.description2 ?? ''}  × 1`);
    try {
      await expect(page.locator(`td.product-total > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`td.product-total > :not(del) .woocommerce-Price-amount > bdi`)).first()).toHaveText(`${vars.unitPrice2 ?? ''}`);
    } catch { /* optional step: assertText */ }
    await expect(page.locator(`tr.cart-subtotal > td .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.unitPrice2 ?? ''}`);
    await expect(page.locator(`tr.order-total > td .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.unitPrice2 ?? ''}`);
    try {
...
Please use a password with at least 8 characters.`);
    ...
    try { await page.locator(`#billing_last_name`).first().fill(`${vars.lastName ?? ''}`); } catch { await page.locator(`#billing_last_name`).first().selectOption(`${vars.lastName ?? ''}`); }
    try {
      try { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="cardnumber"]`).first().fill(`4242424242424242`); } catch { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="cardnumber"]`).first().selectOption(`4242424242424242`); }
    } catch { /* optional step: assign */ }
    try {
      try { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="exp-date"]`).first().fill(`04 / 27`); } catch { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="exp-date"]`).first().selectOption(`04 / 27`); }
    } catch { /* optional step: assign */ }
    try {
      try { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="cvc"]`).first().fill(`234`); } catch { await page.locator(`iframe[src*="js.stripe.com"]`).first().contentFrame().locator(`input[name="cvc"]`).first().selectOption(`234`); }
    } catch { /* optional step: assign */ }
    ...
    await page.waitForLoadState('load');
    vars.orderNumber = ((await page.locator(`.order > strong`).textContent()) ?? '').trim();
    await expect(page.locator(`.email > strong`).first()).toHaveText(`${vars.email2 ?? ''}`);
    ...
    await expect(page.locator(`h3.os-course__title > a[href*="/courses/"]`).first()).toContainText(`${vars.description2 ?? ''}`);
    await expect(page.locator(`td.woocommerce-table__product-name`).first()).toHaveText(`${vars.description2 ?? ''} × 1`);
    await expect(page.locator(`td.woocommerce-table__product-total > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.unitPrice2 ?? ''}`);
