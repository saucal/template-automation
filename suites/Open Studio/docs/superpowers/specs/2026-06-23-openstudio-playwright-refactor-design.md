...
| Email | `helpers/email.ts` interface exposing `forgotPasswordLink` / `refundEmail` / `welcomeLink`. Actual source — SAU/CAL Playgrounds Email Redirect plugin (as No Pong) **vs** GI hosted inbox `https://email.ghostinspector.com/{user}` (what GI tests used) — confirmed at build by inspecting the live staging site. Interface is stable either way. |
