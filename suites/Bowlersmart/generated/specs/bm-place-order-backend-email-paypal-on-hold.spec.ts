// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "BM - Place order Backend + Email - PayPal (on hold)"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { checkOrderOnMyAccount, exitTestProdSite, extractUserFromEmail, fillCheckout, loginAdmin, selectPayPal } from '../helpers/bm-common-steps';
...
test.describe('BM - Place order Backend + Email - PayPal (on hold)', () => {
  ...
  test('02 - Place order - PayPal - 01 - New User', async ({ page }) => {
    await page.goto(`/`);
...
return total === total2 }, vars)).toBeTruthy();
  ...
  test('02 - Place order - PayPal - 02 - Email', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
...
${vars.countryComplete ?? ''}`);
  ...
  test('02 - Place order - PayPal - 03 - Backend', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
...
return shipping !== null }, vars)) {
    ...
    if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); const shipping = document.querySelector<HTMLTableRowElement>('tr.fee > td.line_cost > .view > .woocommerce-Price-amount.amount')

return shipping !== null  }, vars)) {
      vars.FeeWithoutTax = ((await page.locator(`tr.fee > td.line_cost > .view > .woocommerce-Price-amount.amount`).textContent()) ?? '').trim();
    }
    if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); const shipping = document.querySelector<HTMLTableRowElement>('tr.fee > td.line_cost > .view > .woocommerce-Price-amount.amount')

return shipping !== null  }, vars)) {
      try { await page.locator(`tr.fee > td.line_cost > .refund > input.refund_line_total.wc_input_price`).first().fill(`${vars.FeeWithoutTax ?? ''}`); } catch { await page.locator(`tr.fee > td.line_cost > .refund > input.refund_line_total.wc_input_price`).first().selectOption(`${vars.FeeWithoutTax ?? ''}`); }
    }
    if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); const shipping = document.querySelector<HTMLTableRowElement>('tr.fee > td.line_tax > .view > .woocommerce-Price-amount.amount')

return shipping !== null }, vars)) {
      vars.feeTax = ((await page.locator(`tr.shipping > td.line_tax > .view > .woocommerce-Price-amount.amount`).textContent()) ?? '').trim();
    }
    if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); const shipping = document.querySelector<HTMLTableRowElement>('tr.fee > td.line_tax > .view > .woocommerce-Price-amount.amount')

return shipping !== null }, vars)) {
      try { await page.locator(`tr.shipping > td.line_tax > .refund > input.refund_line_tax.wc_input_price`).first().fill(`${vars.feeTax ?? ''}`); } catch { await page.locator(`tr.shipping > td.line_tax > .refund > input.refund_line_tax.wc_input_price`).first().selectOption(`${vars.feeTax ?? ''}`); }
    }
    ...
    await expect(page.locator(`tr:nth-of-type(1) > td.total.refunded-total > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.total ?? ''} USD`);
...
return hasPayPalRefundNote() }, vars)).toBeTruthy();
  ...
  test('05 - Refund Email', async ({ page }) => {
    ...
    await expect(page.locator(`tfoot > tr:nth-of-type(1) > td > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.total ?? ''}`);
    await expect(page.locator(`tr:nth-of-type(7) > td > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`-${vars.total ?? ''}`);
    await expect(page.locator(`tfoot > tr > td > del`).first()).toHaveText(`${vars.total ?? ''}`);
    await expect(page.locator(`tfoot > tr > td > ins > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.Symbol ?? ''}0.00`);
  });
