...
export async function checkoutSection2(page: Page, vars: Record<string, string> = {}): Promise<void> {
  await blockUI(page, vars);
  await expect(page.locator(`ul#shipping_method > li:nth-child(1) label`).first()).toContainText(`Local pickup in Pasco, WA (Call to schedule a pickup 844-534-4274)`);
  await expect(page.locator(`ul#shipping_method > li:nth-child(2) label`).first()).toHaveText(`Free Shipping`);
  vars.shippingPrice = ((await page.locator(`#shipping_method > li:nth-child(1) div > span.woocommerce-Price-amount.amount > bdi`).or(page.locator(`#shipping_method > li:nth-child(1) div.wfacp_shipping_price`)).textContent()) ?? '').trim();
  if (vars.role === 'contractor' && vars.shippingPrice !== 'Free') {
...
${vars.base ?? ''} ${vars.highlight ?? ''}`);
  ...
  await expect(page.locator(`tr.shipping_total_fee span > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`tr.shipping_total_fee > td:nth-child(2) > span`)).first()).toHaveText(`${vars.shippingPrice ?? ''}`);
  if (vars.role === 'contractor' && vars.shippingPrice !== 'Free') {
...
return role === 'contractor' })()) {
  ...
  await page.goto(`${vars.startUrl ?? ''}/wp-cron.php`);
  await page.waitForLoadState('load');
