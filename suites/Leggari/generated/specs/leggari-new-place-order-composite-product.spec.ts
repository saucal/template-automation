...
test.describe('Leggari New - Place Order Composite Product', () => {
  ...
  test('01 - Place order - New User', async ({ page }) => {
    ...
    await page.waitForLoadState('load');
    // ↑ end 02 - Floor kits
    await page.locator(`a[href*="/product/"]:not(a[href*="custom"]) > div:nth-of-type(1) > img`).or(page.locator(`a[href*="/product/"]:not(a[href*="custom"]) > img`)).filter({ visible: true }).first().click({ force: true });
    await page.waitForLoadState('load');
    // ↑ end 03 - Product page
    {
...
wholePrice = parseFloat(wholePrice.replace(',','').replace(`${vars.Symbol}`,'').trim())
...
${vars.email ?? ''}`);
    if (!vars.shippingDesc.includes('pickup')) {
...
return shipping !== null }, vars)) {
    ...
    await expect(page.locator(`li.note.system-note:nth-of-type(2) > .note_content p`).or(page.locator(`li.note.system-note:nth-of-type(3) > .note_content p`)).or(page.locator(`li.note.system-note:nth-of-type(4) > .note_content p`)).first()).toContainText(`Authorize.Net Credit Card Void in the amount of ${vars.total ?? ''} approved. (Transaction ID`);
  });
