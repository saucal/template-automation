...
return test != "preorder" && payment != "ach" })()) {
    await expect(page.locator(`li.note.system-note > .note_content p`).or(page.locator(`li.note.system-note > .note_content p`)).first()).toContainText(`Bluesnap transaction complete (Transaction ID: ${vars.transaction_id ?? ''})`);
  }
  if ((() => { let test = vars.test
let payment =vars.payment
return test != "preorder" && payment != "ach" })()) {
  ...
  if ((() => { let pay = vars.pay
return pay === "now" })()) {
    await expect(page.locator(`li.note.system-note > .note_content p`).first()).toContainText(`Bluesnap transaction complete (Transaction ID: ${vars.transaction_id ?? ''})`);
  }
  if ((() => { let pay = vars.pay
return pay === "upfront" })()) {
    await expect(page.locator(`li.note.system-note > .note_content p`).or(page.locator(`li.note.system-note > .note_content p`)).first()).toContainText(`Bluesnap transaction complete (Transaction ID: ${vars.transaction_id ?? ''})`);
...
export async function completePreOrderWithAdmin(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); return !document.querySelector<HTMLButtonElement>('td.actions.column-actions > button[title="Complete pre-order"].button.complete-order > .button-text') }, vars)) {
    await page.locator(`input[name="order_id[]"][type="checkbox"][value="${vars.orderNumber ?? ''}"]`).or(page.locator(`tr:nth-of-type(${vars.nth ?? ''}) > .check-column > input[name="order_id[]"][type="checkbox"]`)).or(page.locator(`tr:nth-of-type(${vars.nth ?? ''}) > th.check-column > input[name="order_id[]"][type="checkbox"]`)).filter({ visible: true }).first().click({ force: true });
  }
  if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); return !document.querySelector<HTMLButtonElement>('td.actions.column-actions > button[title="Complete pre-order"].button.complete-order > .button-text') }, vars)) {
    {
...
export async function loginAdmin(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  try { await page.locator(`#user_pass`).first().fill(`YWpLs_76f!ePqyhv9P!.`); } catch { await page.locator(`#user_pass`).first().selectOption(`YWpLs_76f!ePqyhv9P!.`); }
  {
...
export async function payPalBlueSnapTemplate(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await page.locator(`#one-time-cta > div > div > div.pe-2`).or(page.locator(`xpath=//button[@id='one-time-cta']/div/div/div[contains(text(), "Pay")]`)).or(page.locator(`#one-time-cta > div > span`)).or(page.locator(`xpath=//button[@id='one-time-cta']/div/span[contains(text(), "Pay")]`)).filter({ visible: true }).first().click({ force: true });
...
return renew
  ...
  await getWooOrderDetails(page, vars);
  await expect(page.locator(`.woocommerce-order-data__meta`).first()).toContainText(`Payment via ${vars.paymentMethodTitle ?? ''} (${vars.transaction_id ?? ''})`);
  await expect(page.locator(`li.note.system-note:nth-of-type(4) > .note_content p`).or(page.locator(`li.note.system-note:nth-of-type(2) > .note_content p`)).first()).toContainText(`Bluesnap transaction complete (Transaction ID: ${vars.transaction_id ?? ''})`);
  if ((() => { let payment = vars.payment
return payment === "ach" })()) {
    await expect(page.locator(`li.note.system-note:nth-of-type(3) > .note_content p`).first()).toContainText(`Charge pending confirmation from BlueSnap (Charge ID: ${vars.transaction_id ?? ''}). Order status changed from Pending payment to On hold.`);
  }
}
