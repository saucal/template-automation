...
export async function bLU001048(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await getGroupLogRequestResponseTransactions(page, vars);
  await expect(page.locator(`li.note.system-note:nth-of-type(2) > .note_content p`).first()).toContainText(`Bluesnap charge complete (Charge ID: ${vars.transaction_id ?? ''})`);
...
export async function bLU001049(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await expect(page.locator(`li.note.system-note:nth-of-type(5) > .note_content p`).first()).toContainText(`Pre-Authorization Released`);
  await verifyLogsTRAUTHREVERSALCAPTURE(page, vars);
