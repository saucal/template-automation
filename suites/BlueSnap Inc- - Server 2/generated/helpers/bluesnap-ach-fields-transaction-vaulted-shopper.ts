...
return logs[1].content.referenceNumber === `${vars.transaction_id}` }, vars)).toBeTruthy();
  ...
  await expect(page.locator(`li.note.system-note:nth-of-type(2) > .note_content p`).first()).toContainText(`BlueSnap ACH Declined ${vars.labeIPN ?? ''} request received. A chargeback has been created for this transaction. Reason: null – null. Order status changed from On hold to Failed.`);
