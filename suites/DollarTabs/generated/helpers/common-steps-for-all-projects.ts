...
return project == "Phlearn" || project == "icg" || vars.project === "vesica" })()) {
  ...
  await expect(page.locator(`#password_1`).or(page.locator(`iframe.phlearn-iframe #password_1`))).not.toHaveCount(0);
  await expect(page.locator(`#password_1`).or(page.locator(`iframe.phlearn-iframe #password_1`)).first()).toBeVisible();
...
let step = vars.step
return  (project === "leggari" && step === "2") })()) {
    await expect(page.locator(`ul#shipping_method > li:nth-child(1) label`).first()).toHaveText(`Local pickup in Pasco, WA (Call to schedule a pickup 844-534-4274)`);
  }
  if ((() => { let project = vars.project
let step = vars.step
return project === "leggari" && step === "3" })()) {
