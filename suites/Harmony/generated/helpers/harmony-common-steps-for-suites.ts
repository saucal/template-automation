...
export async function fillCheckout(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  try { await page.locator(`input.select2-search__field`).first().fill(`${vars.stateComplete ?? ''}`); } catch { await page.locator(`input.select2-search__field`).first().selectOption(`${vars.stateComplete ?? ''}`); }
  ...
  await expect(page.locator(`td.product-total > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`td.product-total > span > span.woocommerce-Price-amount.amount > bdi`)).first()).toHaveText(`${vars.subtotal ?? ''}`);
  if (await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); return !!document.querySelector('.recurring-total') }, vars)) {
    await expect(page.locator(`.wcs-checkout-item-price > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.unitPrice ?? ''}`);
  }
  if (vars.signUpFee !== '') {
    await expect(page.locator(`td.product-total > span > span.subscription-details > span > bdi`).or(page.locator(`.woocommerce-subscriptions-product-details__signup-fee > .woocommerce-Price-amount.amount`)).first()).toHaveText(`${vars.signUpFee ?? ''}`);
  }
  if (vars.country !== 'CA') {
    await blockUI(page, vars);
  }
...
function addMonth(dt) {
  const t = dt.m;
  const y = dt.y + Math.floor(t / 12);
  const m = t % 12 + 1;
  const last = new Date(Date.UTC(y, m, 0)).getUTCDate();
  return { y, m, d: Math.min(dt.d, last) };
}

function parse(s) {
  const d = new Date(String(s).trim() + ' UTC');
  if (isNaN(d)) throw new Error('Fecha inválida: ' + s);
  return { y: d.getUTCFullYear(), m: d.getUTCMonth() + 1, d: d.getUTCDate() };
}

const pay = parse(`${vars.nextPay}`);
const exp = addMonth(today());

return exp.y === pay.y && exp.m === pay.m && exp.d === pay.d;
