...
export async function _04Step2AffiliateApproval(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await page.locator(`xpath=//span[contains(text(),'${vars.user ?? ''}')]`).filter({ visible: true }).first().click({ force: true });
  {
    const _lbl = page.locator(`label[for="status"]`).filter({ visible: true });
    if (await _lbl.count() > 0) { await _lbl.first().click(); }
    else { await page.locator(`#status`).filter({ visible: true }).first().click({ force: true }); }
  }
  try { await page.locator(`#status`).first().fill(`active`); } catch { await page.locator(`#status`).first().selectOption(`active`); }
  {
  ...
  await expect(page.locator(`.updated > p:nth-of-type(1)`).first()).toContainText(`Affiliate updated successfully`);
  await expect(page.locator(`#status`).first()).toContainText(`active`);
}
...
export async function _05PlaceOrder02OneTimeOrderBackend(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await page.locator(`.wp-list-table tr#post-${vars.orderNumber ?? ''} td.order_number a.order-view`).or(page.locator(`.wp-list-table tr#post-${vars.orderNumber ?? ''} th.order_number a.order-view`)).filter({ visible: true }).first().click({ force: true });
...
return comission }, vars);
  vars.noteReferral = ((await page.locator(`#woocommerce-order-notes > div.inside > ul > li:nth-child(7) > div p`).or(page.locator(`#woocommerce-order-notes > div.inside > ul > li:nth-child(3) > div p`)).textContent()) ?? '').trim();
  if (vars.country === 'CA') {
    expect(await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); let note = `${vars.noteReferral}`;

const nums = note.match(/([0-9]+\.[0-9]+)/g).map(Number);
let referral1 = Number(`${vars.referral1}`)

const [referral, reportedUSD, rate] = nums;

const calculated = referral * rate;               // 419.895125
const diff = Math.abs(calculated - reportedUSD);  // 0.005125

const ok = Math.abs(referral1 - referral) < 1e-9   // exact match of the CAD amount
        && diff < 0.01;                 // allow < 0.01 cent
...
export async function _05PlaceOrder03ExistentUserSubscription(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await expect(page.locator(`.subscription-price > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`.product-subtotal > .woocommerce-Price-amount.amount > bdi`)).first()).toHaveText(`${vars.unitPrice ?? ''}`);
...
return subtotal === unit }, vars)).toBeTruthy();
  ...
  await expect(page.locator(`.subscription-price > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`.product-total > .woocommerce-Price-amount.amount > bdi`)).first()).toHaveText(`${vars.unitPrice ?? ''}`);
...
export async function _05PlaceOrder05NewUserSubscription(page: Page, vars: Record<string, string> = {}): Promise<void> {
  ...
  await expect(page.locator(`td.product-price > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.unitPrice ?? ''}`);
  if (vars.country === 'CA') {
    await expect(page.locator(`.woocommerce-subscriptions-product-details__signup-fee > .woocommerce-Price-amount.amount`).first()).toHaveText(`${vars.signUpFee ?? ''}`);
  }
  vars.subtotal = ((await page.locator(`tr.cart-subtotal > td > .woocommerce-Price-amount.amount > bdi`).textContent()) ?? '').trim();
  await expect(page.locator(`.subscription-price > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`.product-subtotal > .woocommerce-Price-amount.amount > bdi`)).first()).toHaveText(`${vars.subtotal ?? ''}`);
  if (vars.country === 'CA') {
    await expect(page.locator(`.subscription-price > .subscription-details > .woocommerce-Price-amount.amount > bdi`).or(page.locator(`.woocommerce-subscriptions-product-details__signup-fee > .woocommerce-Price-amount.amount`)).first()).toHaveText(`${vars.signUpFee ?? ''}`);
  }
  expect(await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); let unit = `${vars.unitPrice}`;
...
ref1 = ref1.parentElement
ref1 = ref1.querySelector('.amount.column-amount > span')
