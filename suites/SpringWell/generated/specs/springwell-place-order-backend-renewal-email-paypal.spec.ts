// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "SpringWell - Place order Backend + Renewal + Email - PayPal"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { blockUI } from '../helpers/common-steps-for-all-projects';
import { acceptCookiesAndClosePopup, backendOrder, checkOrderAndSubscriptionsOnMyAccount, checkTheTotal, exitTest, extractUserFromEmail, fillCheckout, fillPayPal, loginAdmin, renewalOrder } from '../helpers/springwell-common-steps';
...
test.describe('SpringWell - Place order Backend + Renewal + Email - PayPal', () => {
  ...
  test('01 - Place order - New User', async ({ page }) => {
    ...
    await acceptCookiesAndClosePopup(page, vars);
...
return element !== null && element !== undefined }, vars)) {
    ...
    vars.discount = ((await page.locator(`li:nth-child(2) > label > span > span.wcsatt-sub-discount`).textContent()) ?? '').trim();
    vars.unitPrice = await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); const str = `${vars.discount}`;

// 1. Most modern & clean (recommended)
const percentage = (100 - Number(str.replace("%", ""))) / 100;

const unitPrice = (parseFloat(`${vars.unitPrice}`.replace('$','')) * percentage).toFixed(2)
...
return '$'+unitPrice
    ...
    await expect(page.locator(`li:nth-child(2) .price > .woocommerce-Price-amount.amount`).first()).toHaveText(`${vars.unitPrice ?? ''}`);
    vars.freq = ((await page.locator(`li:nth-child(2) .subscription-details`).textContent()) ?? '').trim();
    await page.locator(`td.product-price > ul > li:nth-child(2) > label > input[type=radio]`).filter({ visible: true }).first().click({ force: true });
    ...
    await expect(page.locator(`tr.cart_item:nth-of-type(1) > td.product-name`).first()).toContainText(`${vars.variable1 ?? ''}`);
    await expect(page.locator(`tr.cart_item:nth-of-type(1) > td.product-name`).first()).toContainText(`${vars.variable2 ?? ''}`);
    ...
    await expect(page.locator(`#order_review > table > tbody > tr > td.product-total > span > span.subscription-details`).first()).toHaveText(`${vars.freq ?? ''}`);
    await expect(page.locator(`tr.cart-subtotal > td > .woocommerce-Price-amount.amount > bdi`).first()).toHaveText(`${vars.subtotal ?? ''}`);
    ...
    // TODO: command="assertNotText" target="tr.tax-total > td > .woocommerce-Price-amount.amount > bdi" value="$0.00"
    expect(await _giEval(page, (vars: any) => { vars = new Proxy(vars, { get: (o, k) => (k in o ? o[k] : '') }); const taxPrice = parseFloat(`${vars.taxPrice}`.replace('$',''))
...
return taxPrice !== 0 }, vars)).toBeTruthy();
    ...
    await expect(page.locator(`tr.tax-total.recurring-total > td`).first()).toHaveText(`${vars.taxPrice ?? ''} ${vars.freq ?? ''}`);
  ...
  test('02 - Place order - Backend + Renewal', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
    ...
    await expect(page.locator(`tr#post-${vars.orderNumber ?? ''} > td.order_total.column-order_total .woocommerce-Price-amount.amount`).or(page.locator(`tr.iedit.author-other.level-0.type-shop_order.status-wc-processing.hentry:nth-of-type(1) > td.order_total.column-order_total .woocommerce-Price-amount.amount`)).first()).toHaveText(`${vars.total ?? ''}`);
