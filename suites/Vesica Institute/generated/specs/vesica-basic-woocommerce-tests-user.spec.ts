...
test.describe('Vesica - Basic WooCommerce Tests - User', () => {
  ...
  test('01 - Place order - New User - CC', async ({ page }) => {
    ...
    await page.goto(`${vars.startUrl ?? ''}product/minerals-crystals-for-times-of-stress-creating-vibrational-health-stability-empowerment/`);
    await page.waitForLoadState('load');
    ...
    try {
      vars.shippingPrice = ((await page.locator(`#shipping_method > li:nth-child(1) > label > span > bdi`).textContent()) ?? '').trim();
    } catch { /* optional step: extract */ }
    vars.taxPrice = ((await page.locator(`#order_review > table > tfoot > tr.tax-total > td > span > bdi`).textContent()) ?? '').trim();
    ...
    await expect(page.locator(`table.woocommerce-table > tfoot:nth-of-type(1) > tr:nth-of-type(2) > td > .woocommerce-Price-amount.amount`).first()).toHaveText(`${vars.taxPrice ?? ''}`);
    await expect(page.locator(`table.woocommerce-table > tfoot:nth-of-type(1) > tr:nth-of-type(3) > td`).first()).toContainText(`Credit Card`);
    await expect(page.locator(`table.woocommerce-table > tfoot:nth-of-type(1) > tr:nth-of-type(4) > td > .woocommerce-Price-amount.amount`).first()).toHaveText(`${vars.total ?? ''}`);
    try {
...
${vars.email ?? ''}`);
  ...
  test('01 - Place order - New User - CC - backend', async ({ page }) => {
    ...
    await page.waitForLoadState('load');
    await expect(page.locator(`h2.woocommerce-order-data__heading`).first()).toContainText(`Order #${vars.orderNumber ?? ''}`);
...
${vars.phone ?? ''}`);
  ...
  test('02 - Place order - Logged User - PayPal', async ({ page }) => {
    ...
    await page.waitForLoadState('load');
    try {
      await expect(page.locator(`#password_1`)).not.toHaveCount(0);
...
${vars.street2 ?? ''}
${vars.city ?? ''}, ${vars.shortState ?? ''} ${vars.zipCode ?? ''}`);
    await expect(page.locator(`#order_data > div.order_data_column_container > div:nth-child(3) > div:nth-child(2) > p:nth-child(1)`).or(page.locator(`div.order_data_column:nth-of-type(3) > .address > p:nth-of-type(1)`)).first()).toHaveText(`${vars.firstName ?? ''} ${vars.lastName ?? ''}
${vars.company ?? ''}
${vars.street ?? ''}
${vars.street2 ?? ''}
${vars.city ?? ''}, ${vars.shortState ?? ''} ${vars.zipCode ?? ''}`);
