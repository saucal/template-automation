...
const CHAIN_FILE = path.join(__dirname, '..', 'auth', 'chain-purcrystal.json');
...
// ---------------------------------------------------------------------------
// Brand config + constants (this branch = Pur Crystal only)
// ---------------------------------------------------------------------------

export const brandConfig: BrandConfig = {
  product: { slug: 'first-simple-in-meteorite', type: 'simple' },
  hasPayPal: false,
  hasRegister: false,
...
/** GI picks the first in-stock SIMPLE product from the meteorite category; the variable
 *  product (guest test 10) is the first product-type-variable in the same category. */
export const MINERAL_CATEGORY = 'crystals-minerals/meteorite/';

export const ORDER_DETAILS_TABLE =
  'table.woocommerce-table--order-details, table.shop_table.order_details';

/** Default order quantity (GI Pur Crystal orders qty 2). */
...
/** Separate SHIPPING address (GI ships to a different address — company2 / street3). */
export const SHIPPING: Omit<BillingAddress, 'email'> = {
  ...BILLING,
  company: 'Shipping Inc.',
  address2: 'AP. Shipping',
};
...
export async function dismissCookieBanner(page: Page): Promise<void> {
  ...
  // CookieYes (cky-*). Its accept button is #cookie_action_close_header (.wt-cli-accept-btn).
  // ACCEPT (not reject) so consent-gated embeds load and settle for the nav visuals.
  const btn = page
    .locator('#cookie_action_close_header, .wt-cli-accept-btn, .cli_action_button, .cky-btn-accept, button[aria-label="Accept All"]')
...
/**
 * Navigate the meteorite category and open the first in-stock product of `type`
 * (GI picks `product-type-simple`; the variable-product guest test picks
 * `product-type-variable`). Sets the PDP quantity when `qty > 1`. Captures name+price.
 */
export async function pickFirstProduct(
  page: Page,
  categoryPath: string,
  opts: { qty?: number; type?: 'simple' | 'variable' } = {}
): Promise<PdpCapture> {
  const { qty = 1, type = 'simple' } = opts;
  await page.goto(categoryPath, { waitUntil: 'load' });
  await dismissCookieBanner(page);
  const ctx = ctxFor(page);
  const typeCls = type === 'variable' ? 'product-type-variable' : 'product-type-simple';
  await resilientClick(ctx, {
    primary: page.locator(`li.product.instock.${typeCls} a.woocommerce-loop-product__link`).first()
      .or(page.locator(`li.instock.${typeCls} > a`).first()),
    alt: page.locator(`li.product.instock.${typeCls} a`).first(),
    ai: `the first in-stock ${type} product in the listing`,
  });
  ...
  if (qty > 1) {
    await page.locator('input.qty, input[name="quantity"]').first()
      .evaluate((el, v) => { (el as HTMLInputElement).value = String(v); el.dispatchEvent(new Event('change', { bubbles: true })); }, qty)
      .catch(() => {});
  }
...
export function calcSubtotal(unitPrice: string, qty: number): number {
  return Math.round(toAmount(unitPrice) * qty * 100) / 100;
}
...
/** Fill + submit the Elementor contact form (GI guest test 08). */
 ...
 */
export async function fillShippingAddress(page: Page, shipping: Omit<BillingAddress, 'email'>): Promise<void> {
  const toggle = page.locator('#ship-to-different-address-checkbox').first();
  if (!(await toggle.isChecked().catch(() => false))) {
    await toggle.check().catch(async () => {
      await page.locator('label[for="ship-to-different-address-checkbox"]').click({ force: true }).catch(() => {});
    });
  }
  await waitForCheckoutReady(page);
  const ctx = ctxFor(page);
  const text: Array<[string, string | undefined, string]> = [
    ['#shipping_first_name', shipping.firstName, 'the shipping first name field'],
    ['#shipping_last_name', shipping.lastName, 'the shipping last name field'],
    ['#shipping_company', shipping.company, 'the shipping company field'],
    ['#shipping_address_1', shipping.address, 'the shipping street address field'],
  ...
  await setSelect(page, '#shipping_country', shipping.shortCountry ?? shipping.country ?? 'US');
  await waitForCheckoutReady(page);
  await setSelect(page, '#shipping_state', shipping.shortState ?? shipping.state);
  await waitForCheckoutReady(page);
}
