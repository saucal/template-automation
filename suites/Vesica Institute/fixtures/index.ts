...
const REJECTION_GUARD_FLAG = '__plsStagehandRejectionGuard';
...
export const test = base.extend<Fixtures>({
  ...
  shopperPage: async ({ pageBrowser }, use, testInfo) => {
    ...
    // Priming visit with the SAU/CAL captcha-bypass param (?sc_bypass=1): if the
    // bypass plugin is active it sets the server-side flag for this context so
    // later form submits skip the captcha; if not, the param is ignored — either
    // way this is just the initial page load. Relative goto resolves against the
