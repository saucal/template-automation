// @ts-nocheck
// Auto-generated from Ghost Inspector suite: "Botany Farms - Basic WooCommerce Test"
// Review all TODOs before running.
import dotenv from 'dotenv';
dotenv.config();
import { test, expect } from '@playwright/test';
import { login, register } from '../helpers/botany-farms-common-steps-for-suites';
import { blockImageSizes, blockUI, calculateSubtotal, extractUserFromEmail, login, placeOrderElement, wooCommerceCheckoutTemplate } from '../helpers/common-steps-for-all-projects';
...
test.describe('Botany Farms - Basic WooCommerce Test', () => {
  ...
  test('01 - Home Page', async ({ page }) => {
    ...
    await page.locator(`xpath=//span[contains(text(), "Accept")]`).or(page.locator(`button[type="submit"] > span > .elementor-button-text`)).filter({ visible: true }).first().click({ force: true });
    await page.waitForLoadState('load');
    try {
      await page.locator(`xpath=//button[contains(text(), "NO THANKS")]`).or(page.locator(`div[data-testid="form-row"].needsclick:nth-of-type(7) > [data-testid="form-component"].needsclick > button[type="button"].needsclick`)).or(page.locator(`div.needsclick:nth-of-type(7) > .needsclick > button[type="button"].needsclick`)).filter({ visible: true }).first().click({ force: true });
    } catch { /* optional step: click */ }
    await blockImageSizes(page, vars);
    {
      const _lbl = page.locator(`label[for="agl_yes_button"]`).filter({ visible: true });
      if (await _lbl.count() > 0) { await _lbl.first().click(); }
      else { await page.locator(`#agl_yes_button`).filter({ visible: true }).first().click({ force: true }); }
    }
  });
  ...
  test('02 - Shop Page', async ({ page }) => {
    ...
    await page.locator(`.menu-item > a[href*="/shop/"]`).filter({ visible: true }).first().click({ force: true });
    await page.waitForLoadState('load');
    await page.locator(`button[type="submit"] > span > .elementor-button-text`).filter({ visible: true }).first().click({ force: true });
    try {
      await page.locator(`div[data-testid="form-row"].needsclick:nth-of-type(7) > [data-testid="form-component"].needsclick > button[type="button"].needsclick`).or(page.locator(`div.needsclick:nth-of-type(7) > .needsclick > button[type="button"].needsclick`)).filter({ visible: true }).first().click({ force: true });
    } catch { /* optional step: click */ }
  ...
  test('03 - Product Page', async ({ page }) => {
    ...
    await expect(page.locator(`.yotpo-bottomline`)).not.toHaveCount(0);
    // ↑ end 02 - Shop Page
    await page.locator(`a[href*="/product/"] > img.attachment-woocommerce_thumbnail.size-woocommerce_thumbnail`).filter({ visible: true }).first().click({ force: true });
    await page.waitForLoadState('load');
    try {
      await page.locator(`xpath=//button[contains(text(), "NO THANKS")]`).or(page.locator(`div[data-testid="form-row"].needsclick:nth-of-type(7) > [data-testid="form-component"].needsclick > button[type="button"].needsclick`)).or(page.locator(`div.needsclick:nth-of-type(7) > .needsclick > button[type="button"].needsclick`)).filter({ visible: true }).first().click({ force: true });
    } catch { /* optional step: click */ }
    ...
    await expect(page.locator(`.yotpo-stars`)).not.toHaveCount(0);
    await page.locator(`.yotpo-stars > span.yotpo-icon.yotpo-icon-star.pull-left:nth-of-type(3)`).filter({ visible: true }).first().click({ force: true });
    await expect(page.locator(`#tab-title-yotpo_widget`)).not.toHaveCount(0);
    await expect(page.locator(`.yotpo.yotpo-main-widget`)).not.toHaveCount(0);
    await expect(page.locator(`.yotpo-review`)).not.toHaveCount(0);
  });
  ...
  test('06 - Place order - New User - 01 - Frontend', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
...
${vars.city ?? ''}, ${vars.state ?? ''} ${vars.zipCode ?? ''}`);
  ...
  test('06 - Place order - New user - 02 - Backend', async ({ page }) => {
    await page.goto(`/`);
    await page.waitForLoadState('load');
...
return site }, vars);
    ...
    await page.locator(`xpath=//a[contains(text(), "Your Botany Farms order")]`).filter({ visible: true }).first().click({ force: true });
...
${vars.city ?? ''}, ${vars.state ?? ''} ${vars.zipCode ?? ''}`);
    ...
    vars.pass = `0*qYbn6rCIzc3aERF0c*0bIt`;
